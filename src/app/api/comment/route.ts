...
export async function POST(req: NextRequest) {
  try {
    ...
    if (data.chapterID && data.comicsID)
      return NextResponse.json(
        { message: `Bad Request: Can not have both chapterID and comicsID` },
        { status: 400 }
      );
    ...
    if (comic === null)
      return NextResponse.json(
        { message: `Not Found: Can not find comic` },
        { status: 404 }
      );
    ...
    if (chapter === null)
      return NextResponse.json(
        { message: `Not Found: Can not find comic chapter` },
        { status: 404 }
      );

    const commentReply = !data.commentID
      ? undefined
    ...
    if (commentReply === null)
      return NextResponse.json(
        { message: `Not Found: Can not find comment reply` },
        { status: 404 }
      );
    ...
    const id = data.commentID || data.comicsID || data.chapterID;
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2023") {
        return NextResponse.json(
          { message: `Some fields are wrong format` },
          { status: 400 }
        );
      }
    }
...
export async function GET(req: NextRequest) {
  //Lấy tất cả các root của comment
  try {
    const comicId =
      req.nextUrl.searchParams.get("ComicId") === "undefined"
        ? undefined
