...
export async function GET(req: NextRequest, context: any) {
  ...
  try {
    ...
    if (isGetRootChain === "true") {
      //Lấy chuỗi root query
...
export async function POST(req: NextRequest, context: any) {
  const queryOptionLike = (isLike: boolean, userID: any) => {
    ...
    return !isLike ? connect : disconnect;
  };
  ...
  try {
    ...
    if (!comment)
      return NextResponse.json(
        { message: `Not Found: Can not find comment` },
        { status: 404 }
      );
...
export async function PATCH(req: NextRequest, context: any) {
  ...
  try {
    ...
    if (profile.id !== comment.userID)
      return NextResponse.json(
        { message: `Forbidden: Can not edit comment with this account` },
        { status: 403 }
      );
...
export async function DELETE(req: NextRequest, context: any) {
  ...
  try {
    ...
    if (profile.id !== comment.userID)
      return NextResponse.json(
        { message: `Forbidden: Can not delete comment with this account` },
        { status: 403 }
      );
